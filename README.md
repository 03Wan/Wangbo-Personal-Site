...
新页面仍需开发并部署。在 `/studio` 的“路由访问管理”中添加路由地址（例如 `/wb`），设置“允许访问”并点击“发布”，无需再为该页面修改访问控制代码。新加的路由默认隐藏；关闭后，该地址及其下级地址直接返回 404，重新开启并发布即可恢复访问。如果新页面从上线起就要隐藏，先在后台添加并发布该路由，再部署页面。`/send-ywy` 的设置也控制 `/zh/send-ywy` 这个旧入口。未加入列表的额外页面默认公开。

这只是网站访问开关。仓库目前公开，已提交的页面代码和 `public/` 文件仍可从 GitHub 获取；如果内容需要真正保密，应将其移出公开仓库和公开静态目录，并使用身份验证保护。
