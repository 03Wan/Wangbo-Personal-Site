...
在 `/studio` 的“临时页面访问控制”中，可以分别关闭 `/friendship` 和 `/send-ywy`，然后点击“发布”。关闭后，相关地址直接返回 404；`/send-ywy/index.html`、`/zh/send-ywy` 及该目录下的图片也会一起关闭。两个开关默认开启，重新开启并发布即可恢复访问。新加临时页面时，需要先在 `src/proxy.ts` 的路由匹配和控制逻辑中登记，再在后台增加相应开关。
