...
export default async function ContactPage({ params }: Props) {
  const { locale } = await params; if (!isLocale(locale)) notFound(); const data = getContent(locale); const extra = siteCopy[locale].contact;
  const subject = locale === "zh" ? "来自个人官网的交流" : "Hello from your portfolio";
  return <div className="section-shell page-shell contact-page"><PageIntro eyebrow={data.contact.eyebrow} title={data.contact.title} lead={data.contact.lead} /><div className="contact-grid"><a className="contact-card" href={`mailto:${data.shared.email}?subject=${encodeURIComponent(subject)}`}><span className="contact-index">01</span><div><span className="eyebrow">EMAIL</span><h2>{data.contact.emailTitle}</h2><p>{data.contact.emailText}</p><strong>{data.shared.email}</strong></div><b>→</b></a><a className="contact-card" href={data.shared.github} target="_blank" rel="noopener noreferrer"><span className="contact-index">02</span><div><span className="eyebrow">GITHUB</span><h2>{data.contact.githubTitle}</h2><p>{data.contact.githubText}</p><strong>github.com/03Wan</strong></div><b>↗</b></a></div><section className="contact-topics"><span className="section-index">03</span><div><h2>{extra.topicsTitle}</h2><ul>{extra.topics.map((topic) => <li key={topic}>{topic}</li>)}</ul></div></section><p className="privacy-note"><span>PRIVACY</span>{data.contact.privacy}</p></div>;
}
