...
import { getProject, projectStatusLabels, projects } from "@/lib/projects";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: content.title, description: content.summary, alternates: { canonical: path, languages: { "zh-CN": `/zh/projects/${slug}`, "en-US": `/en/projects/${slug}` } }, openGraph: { title: content.title, description: content.summary, url: path, locale: locale === "zh" ? "zh_CN" : "en_US", type: "article" } };
}
...
export default async function ProjectDetailPage({ params }: Props) {
  ...
  return <article className="section-shell case-page">
    <header className="case-hero"><div className="case-title"><span className="eyebrow">PROJECT / {String(index + 1).padStart(2, "0")}</span><h1>{content.title}</h1><p>{content.subtitle}</p><div className="tag-list">{tags.map((tag) => <span key={tag}>{tag}</span>)}</div></div><dl className="case-facts"><div><dt>{copy.period}</dt><dd>{locale === "zh" ? project.period : project.periodEn}</dd></div><div><dt>{copy.status}</dt><dd className="status-text">{projectStatusLabels[locale][project.status]}</dd></div><div><dt>{copy.level}</dt><dd>{locale === "zh" ? project.level : project.levelEn}</dd></div><div><dt>{copy.role}</dt><dd>{roles.join(" · ")}</dd></div></dl></header>
    {project.links?.length ? <div className="case-links">{project.links.map((link) => <a key={link.href} href={link.href} target="_blank" rel="noopener noreferrer">{link.label[locale]} ↗</a>)}</div> : null}
    ...
    <section className="case-section evidence-section"><span className="section-index">07</span><div><h2>{copy.evidence}</h2><div className="evidence-placeholder"><span>EVIDENCE NOTE</span><p>{content.evidenceNote}</p></div></div></section>
    ...
    <nav className="case-pagination" aria-label={locale === "zh" ? "项目翻页" : "Project pagination"}><Link href={`/${locale}/projects/${previous.slug}`}><span>← {copy.previous}</span><strong>{previous.localeContent[locale].title}</strong></Link><Link href={`/${locale}/projects`}>{copy.all}</Link><Link href={`/${locale}/projects/${next.slug}`}><span>{copy.next} →</span><strong>{next.localeContent[locale].title}</strong></Link></nav>
  </article>;
}
